...
from gaussian_renderer import render, network_gui, render_feature_woseprate
import sys
...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
                         ...
                         gaussians, scene, stage, tb_writer, train_iter,timer, num_classes = None):
    first_iter = 0
    ...
    count = 0
    if stage == "fine":
        classifier = Classifier(hyper, dataset.feature_dim, num_classes, use_BCE = True)
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        if stage == "fine" and use_dino:
            render_pkg_feature = render_feature_woseprate(viewpoint_cam, gaussians, pipe, bg_color = torch.tensor([1, 0, 0], dtype=torch.float32, device="cuda"), classifier = classifier)
            objects = render_pkg_feature["render"] 
            dino_feat = dino_list[viewpoint_cam.uid].cuda() # torch.Size([1, 1, 137, 77, 768])
            objects_downsampled = F.interpolate(objects.unsqueeze(0), size=(137, 77), mode='bilinear', align_corners=False).squeeze(0)  # torch.Size([25, 137, 77])
            objects_projected = projector(objects_downsampled)  # torch.Size([768, 137, 77])

            objects_norm = F.normalize(objects_projected, dim=1).float()
            dino_norm = F.normalize(dino_feat, dim=1).permute(2, 0, 1).float()
            loss += F.mse_loss(objects_norm, dino_norm)
...
def training(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname, mode, num_classes, use_dino = False, dino_list = None, dino_list_id = None):
    # first_iter = 0
    tb_writer = prepare_output_and_logger(expname)
    gaussians = GaussianModel(dataset.sh_degree, mode, hyper)   # 是否建立分割网络
    dataset.model_path = args.model_path
    timer = Timer()
    scene = Scene(dataset, gaussians, mode=mode, load_coarse=None)      # 此时选择哪种优化器已经由mode决定好了，加载数据和ckpt
    timer.start()
    ...
    scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations,
                         checkpoint_iterations, checkpoint, debug_from,
                         gaussians, scene, "fine", tb_writer, opt.iterations,timer, num_classes)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_checkpoint", type=str, default = "./output/hypernerf/broom-4dgs/chkpnt_coarse_3000.pth")   # ./output/uw/composite/chkpnt_coarse_3000.pth
    parser.add_argument("--expname", type=str, default = "./hypernerf/broom-4dgs")
    parser.add_argument("--configs", type=str, default = "arguments/hypernerf/broom2.py")  # arguments/hypernerf/default.py arguments/dnerf/bouncingballs.py
    ...
    # args.source_path = "data/hypernerf/chicken"
    ...
    training(lp.extract(args), hp.extract(args), op.extract(args), pp.extract(args), 
             ...
             args.start_checkpoint, args.debug_from, args.expname, args.mode, args.num_classes,
             use_dino=use_dino,
