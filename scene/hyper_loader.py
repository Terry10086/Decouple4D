...
class Load_hyper_data(Dataset):
    def __init__(self, 
        ...
        self.object_folder = os.path.join(datadir, "sam2_mask") if object_masks else None    # pseudo_label/object_mask sam2_mask
        # self.num_classes = 0
        ...
        with open(f'{datadir}/metadata.json', 'r') as f:        # 保存了每张图的时间
    ...
    def load_raw(self, idx):
        ...
        if self.object_folder is not None:  # 
            object_path = os.path.join(self.object_folder, image_name)  # '/media/yangtongyu/T9/code2/sa4d-time_variant_ie/data/hypernerf/chickchicken/sam2_mask/000001.png'
            objects = Image.open(object_path) # if os.path.exists(object_path) else None
            if 'sam' in self.object_folder:
                objects = torch.from_numpy((np.array(objects)// 255).astype(np.uint8))
            else:
