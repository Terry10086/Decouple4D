...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, mode="scene", shuffle=True, resolution_scales=[1.0], load_coarse=None, cam_view=None, is_eval = False):
        """
        ...
        if os.path.exists(os.path.join(args.source_path, "sparse")):
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, args.llffhold, object_masks=args.object_masks, need_gt_masks=args.need_gt_masks, is_eval = is_eval)
            dataset_type="colmap"
        ...
        if self.gaussians is not None:
            ...
            if self.loaded_iter:
                ...
                if mode == "scene":
                    ...
                    pc_path = os.path.join(self.model_path, "point_cloud")
                    if os.path.isdir(pc_path):
                        self.loaded_iter = searchForMaxIteration(pc_path)
