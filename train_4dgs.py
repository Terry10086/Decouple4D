...
def scene_reconstruction(dataset, opt, hyper, pipe, testing_iterations, saving_iterations, 
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        loss = 0
        if stage == 'fine':
            dx, ds, dr = render_pkg['dx'].norm(dim=-1, keepdim=True), render_pkg['ds'].norm(dim=-1, keepdim=True), render_pkg['dr']
            loss_dx = dx.mean()
            loss_ds = ds.mean()
            loss += 0.1*(loss_dx + loss_ds)
        ...
        # loss_theta_deg = theta_deg.mean() / 360
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter :
                ...
                if  iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0 and gaussians.get_xyz.shape[0]<250000:
                ...
                if iteration % opt.densification_interval == 0 and gaussians.get_xyz.shape[0]<250000 and opt.add_point:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[ 3000, 7000, 14000, 20000, 30_000, 45000, 60000])
    parser.add_argument("--quiet", action="store_true")
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[3000, 7000, 14000, 20000])  # 真正保存ckpt
    parser.add_argument("--start_checkpoint", type=str, default = "./output/hypernerf/hand1/chkpnt_fine_13999.pth")   # ./output/uw/composite/chkpnt_coarse_3000.pth ./output/robust/statue/chkpnt_coarse_3000.pth
    parser.add_argument("--expname", type=str, default = f"./hypernerf/{scene_name}") # /media/yangtongyu/T9/zyp/sa4d/output/hypernerf/torchocolate  ./uw/composite_2
