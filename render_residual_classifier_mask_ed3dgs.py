...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, classifier, cam_type, dataset_source_path, gru):
    ...
    mask_path = os.path.join(dataset_source_path, "final_mask_e-d3dgs")
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        path = r'/media/yangtongyu/T9/code3/E-D3DGS/output/hypernerf/split-cookie/mask'
        object_path = os.path.join(path, f'{int(idx*3):05d}.png')
        objects = Image.open(object_path)
        objects = torch.from_numpy(np.array(objects)//255).cuda()
        ...
        residual_mask_upper_sam = intersect_masks(conf, sam_masks.cuda(), 0.6)  # 0.3 越高 越bu容易包含东西   chicken 0.4 0.5 0.5 前期可以多一些噪声   0.3 torch
...
if __name__ == "__main__":
    ...
    cmdlne_string = ['--model_path', './output/hypernerf/split-cookie-partial/']    # 也决定了读哪个数据集
