...
from gaussian_renderer import network_gui, render_all, render, render_contrastive_feature
import sys
        ...
        with torch.no_grad():
            ...
            if dataset.render_process:
                if (iteration < 1000 and iteration % 10 == 9) \
                    ...
                    # breakpoint()
                        # TODO: bugs here, no in render return
                        pass
...
def training_init(dataset, hyper, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, expname, mode):
    # first_iter = 0
    tb_writer = prepare_output_and_logger(expname)
    gaussians = GaussianModel(dataset.sh_degree, mode, hyper)       # scene
    dataset.model_path = args.model_path
    timer = Timer()
    scene = Scene(dataset, gaussians, mode=mode, load_coarse=None)      # 此时选择哪种优化器已经由mode决定好了
    timer.start()
...
def training(dataset, hyper, opt, pipe, expname, testing_iterations=None, saving_iterations=None, checkpoint_iterations=None, checkpoint=None, mode="feature", debug_from=None, cam_view=None, timer = None, num_classes = None):
    # stage = 'fine'
    dataset.object_masks = True
    # tb_writer = prepare_output_and_logger(expname)
    num_classes = num_classes
    print("Num classes: ",num_classes)
    gaussians = GaussianModel(dataset.sh_degree, mode, hyper, dataset.feature_dim, num_classes = num_classes)
    scene = Scene(dataset, gaussians, mode=mode, load_coarse=None)  # load_deformation, GS; 如果mode=fea，则额外导入segment & 重新导入一遍数据集，这次多导入了mask
    ...
    for iteration in range(first_iter, final_iter + 1):        
        ...
        # # Pick a random Camera
        # # # dynerf's branch
        # if opt.dataloader and not load_in_memory:       # False
        #     try:
        #         viewpoint_cams = next(loader)
        #     except StopIteration:
        #         print("reset dataloader into random dataloader.")
        ...
        identity_encoding = gaussians._mlp(gaussians.get_xyz, torch.tensor(viewpoint_cam.time).cuda().repeat(gaussians.get_xyz.shape[0], 1))    # torch.Size([315326, 32])
        ...
        render_pkg = render_contrastive_feature(viewpoint_cam, gaussians, pipe, background, prob_obj3d = prob_obj3d)
        objects = render_pkg["render"]      # torch.Size([32, 960, 536])
    ...
    scene.save(scene.loaded_iter)       # 迭代结束后，保存ckpt
    ...
    #     viewspace_point_tensor_list = []
    #     for viewpoint_cam in viewpoint_cams:
    #         render_pkg = render_all(viewpoint_cam, gaussians, pipe, background, stage=stage,cam_type=scene.dataset_type)
    #         image, viewspace_point_tensor, visibility_filter, radii, objects = render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["rendered_feature_map"]
    #         images.append(image.unsqueeze(0))
    #         if scene.dataset_type!="PanopticSports":
    #             gt_image = viewpoint_cam.original_image.cuda()
    #         else:
    ...
    #     image_tensor = torch.cat(images,0)
    #     gt_image_tensor = torch.cat(gt_images,0)
    #     # gt_obj_tensor = torch.cat(gt_objs,0)
    ...
    # scene.save(scene.loaded_iter)
        ...
        #     # Log and save
        #     timer.pause()
        #     training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_all, [pipe, background], stage, scene.dataset_type)
        #     if (iteration in saving_iterations):
        #         print("\n[ITER {}] Saving Gaussians".format(iteration))
        #         scene.save(iteration, stage)
        #     if dataset.render_process:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint_iterations", nargs="+", type=int, default=[3000, 14000, 20000])  # 真正保存ckpt
    parser.add_argument("--start_checkpoint", type=str, default = "")   # ./output/hypernerf/broom2-1/chkpnt_fine_14000.pth
    parser.add_argument("--expname", type=str, default = "./hypernerf/chicken")
    parser.add_argument("--configs", type=str, default = "arguments/hypernerf/chicken.py")  # arguments/hypernerf/default.py arguments/dnerf/bouncingballs.py
    ...
    # args.source_path = "data/hypernerf/broom2"
    ...
    # init GS
    timer = training_init(lp.extract(args), hp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.expname, args.mode)
    
    with open(os.path.join(args.model_path, "feature_cfg_args"), 'w') as cfg_log_f:
        cfg_log_f.write(str(Namespace(**vars(args))))
    training(lp.extract(args), hp.extract(args), op.extract(args), pp.extract(args), expname = args.expname, testing_iterations = args.test_iterations, saving_iterations = args.save_iterations, checkpoint_iterations = args.checkpoint_iterations, mode='feature', cam_view=args.cam_view, timer = timer, num_classes = args.num_classes)
