...
from gaussian_renderer import render, render_seperate1, render_seperate, render_feature
import torchvision
...
def intersect_masks(conf: torch.Tensor, sam_masks: torch.Tensor, intersect_threshold: float):
    ...
    ids_conf2 = (conf == 2).to(torch.float32)  # 转成float

    # 取交集
    # robust_zero_mask = fill_gap(ids_conf2, 15)
    # coarse_mask = robust_zero_mask.unsqueeze(-1)
    # ids_conf2_eroded = erode_mask(ids_conf2, kernel_size=5, iterations=1).squeeze()

    ids_conf2_mask = (conf == 2).clone()
    ids_conf2_cleaned = remove_small_components(ids_conf2_mask, min_area=500)
    ids_conf2 = sam_masks[ids_conf2_cleaned]

    valid_ids = ids_conf2.unique()

    ids_conf1 = sam_masks[conf == 1]
    is_valid = torch.isin(ids_conf1, valid_ids)

    conf_filtered = conf.clone()
    conf_filtered[conf == 1] = torch.where(is_valid,
                                            torch.tensor(2, device=conf.device, dtype=conf.dtype),
                                            torch.tensor(0, device=conf.device, dtype=conf.dtype))
    
    coarse_mask = (conf_filtered == 2).unsqueeze(-1).float()  # [H, W, 1]
...
def intersect_masks1(conf: torch.Tensor, sam_masks: torch.Tensor, intersect_threshold: float):
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, classifier, cam_type, dataset_source_path, gru):
    ...
    with torch.no_grad():
        trajectory_list = []
        # N = len(views)
        # test_indices = list(range(0, N, 8))
        # train_indices = sorted(set(range(N)) - set(test_indices))
        # selected_indices_in_train = list(range(0, len(train_indices), len(train_indices) // 4))
        # selected_indices_in_all = [train_indices[i] for i in selected_indices_in_train]
...
#         trajectory_feats = torch.stack(trajectory_list)
#         hidden = gru(trajectory_feats)
#         identidy_encoing = classifier._mlp(gaussians.get_xyz, hidden)
#         logits3d = classifier._classifier(identidy_encoing.unsqueeze(1).permute(2, 0, 1))
...
#         pred_obj = (category > 0.5) # 到这里classifier都是没有梯度的
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        ...
        st_masks, dy_masks = [], []
        for bg_val in [0.0, 1.0]:
            background = torch.full((3,), bg_val, device="cuda")
            pkg = render_seperate(
                view, gaussians, pipeline, background,
                cam_type=cam_type,
                seperate_mask=pred_obj.squeeze(),
                is_eval=True,
                category=category
            )
            render, st, dy = pkg["render"], pkg["render_st"], pkg["render_dy"]

            st_residual = (view.original_image.cuda() - st).abs().mean(dim=0, keepdim=True)
            dy_residual = (render - dy).abs().mean(dim=0, keepdim=True)

            st_masks.append((st_residual < 2*st_residual.mean()).cuda())    # st_masks.append((st_residual < 4*st_residual.mean()).cuda())
            dy_masks.append((dy_residual < 0.5 * dy_residual.mean()).cuda())

        # 取交集
        st_mask = ~(st_masks[0] & st_masks[1])
        dy_mask = dy_masks[0] & dy_masks[1]
        conf = st_mask.to(torch.int) # + dy_mask.to(torch.int)
        ...
        residual_mask_upper_sam = intersect_masks1(conf.squeeze(0), sam_masks.cuda(), 0.3)  # 0.3 越高 越bu容易包含东西   chicken 0.4 0.5 0.5 前期可以多一些噪声   0.3 torch
...
if __name__ == "__main__":
    ...
    cmdlne_string = ['--model_path', './output/robust/patio-high/']    # 也决定了读哪个数据集
