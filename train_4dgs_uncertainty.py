...
def training_seperation(tb_writer, dataset, hyper, opt, pipe, expname, testing_iterations=None, checkpoint_iterations=None, checkpoint=None, debug_from=None, cam_view=None, timer = None, num_classes = None,  gaussians = None, scene = None, use_BCE=False, use_dino = False, dino_list = None, dino_list_id = None):
    ...
    for iteration in range(first_iter, final_iter+1):        
        ...
        image_tensor, viewspace_point_tensor, visibility_filter, radii, rendered_st= \
            render_pkg["render"], render_pkg["viewspace_points"], render_pkg["visibility_filter"], render_pkg["radii"], render_pkg["rendered_st"]

        Ll1 = l1_loss(image_tensor, gt_image)
        loss += (1.0 - opt.lambda_dssim) * Ll1   # 

        if iteration>1000:
            # uncertainty_loss = (l1_loss(rendered_st.detach(), gt_image)/(2 * render_pkg["rendered_motion"].pow(2))).mean() + torch.log(render_pkg["rendered_motion"]).mean()
            # loss += uncertainty_loss

            residual = torch.abs(image_tensor-rendered_st).mean(dim=0, keepdim=True).permute(1,2,0)
            residual_mask = (residual > residual.mean()).to(residual)

            sam_mask_path = os.path.join('/media/yangtongyu/T9/code2/sa4d-time_variant_ie/data/hypernerf/chickchicken/sam_mask_npy', f"train_{viewpoint_cam.uid:06d}.npy")
            sam_mask = torch.from_numpy(np.load(sam_mask_path))
            residual_mask_upper_sam = intersect_masks(residual_mask, sam_mask.cuda().unsqueeze(-1), 0.2)  # 越低 越容易包含东西

            robust_zero_mask = fill_gap(residual_mask_upper_sam, 5)

            mask = (robust_zero_mask != 0).squeeze().unsqueeze(0)  # [1, 960, 536]
            render_pkg["rendered_motion"] = render_pkg["rendered_motion"] * mask  # broadcasting 到 [3, 960, 536]

            loss += render_pkg["rendered_motion"].mean()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start_checkpoint", type=str, default = "./output/hypernerf/chicken1/chkpnt_coarse_3000.pth")   # ./output/hypernerf/chicken-loss_all_woobj3d/chkpnt_coarse_3000.pth
    parser.add_argument("--is_continue", type=bool, default = False)   # 是否导入.pth
    parser.add_argument("--expname", type=str, default = "./hypernerf/chicken1")
