...
def quat_rotate(q, v):
    ...
    returns: rotated vector of shape (..., 3)
    """
    ...
    t = 2 * torch.cross(q_xyz, v, dim=-1)               # t = 2*(q.xyz × v)
    rotated_v = v + q_w * t + torch.cross(q_xyz, t, dim=-1)

    return rotated_v


def loss_cls_3d(features, predictions, k=5, lambda_val=2.0, max_points=200000, sample_size=800, final_mask = None):
    """
    ...
    selected_indices = (final_mask > 0).nonzero(as_tuple=True)[0] 
    ...
    if selected_indices.numel() > sample_size:
        indices = selected_indices[torch.randperm(selected_indices.numel())[:sample_size]]
    else:
        indices = selected_indices

    sample_features = features[indices]         # [S, D]
    sample_preds = predictions[indices]         # [S, C]
    ...
    diff = (sample_preds.unsqueeze(1) - neighbor_preds) ** 2  # [S, k, 1]
    loss = diff.mean()
